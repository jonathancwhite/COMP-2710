 ...
 * email: jcw0057@auburn.edu
 * 
 * project3_White_jcw0057.cpp
 * Compile using g++ compiler and run.
 * 
 */
...
void test_readfile(void) {
    ...
    cout << "\tCase 1.a: a text file that can be found (input0.txt)\n";

    ifstream input;
    input.open("input0.txt");
    int a = readfile(test, input);
    assert((0 <= a));
    cout << "\tCase 1.a passed.\n";
    input.close();
    cout << "\tCase 1.b: a text file that can be found (input3.txt)\n";
    string z = "input3.txt";
    input.open(z.c_str());
    int b = readfile(test, input);
    assert(0 <= b);
    cout << "\tCase 1.b passed.\n";
    input.close();
    cout << "\tCase 1.c: file with a large ammount of inputs (input4.txt)\n";
    input.open("input4.txt");
    int c = readfile(test, input);
    assert(100 == c);
    cout << "\tCase 1.c passed.\n\n";
    input.close();
...
void test_sort(void) {
    ...
    cout << "\tCase 2.a: sorting with an empty array and a filled one\n";
    assert(4 == sort(s1, 0, s2, 4, s5));
    cout << "\tCase 2.a passed.\n";
    cout << "\tCase 2.b: sorting with a unsorted array and reverse order array\n";
    assert(11 == sort(s3, 6, s4, 5, s5));
    cout << "\tCase 2.b passed.\n";
    cout << "\tCase 2.c: sorting with a large unsorted array\n";
    assert(106 == sort(s4, 7, s6, 99, s5));
    cout << "\tCase 2.c passed.\n\n";
    press_any_key(); //Press Any Key To Continue
    cout << endl;
}
// main fuction
int main() {
    ...
    cout << "*** Welcome to Jc's sorting program ***\n";
